...
class App extends Component {
  ...
  initializeDataTable(dataObj) {
    ...
    dataTableContainer.style.width = `${window.innerWidth -
      verticalScrollBarContainer.offsetHeight}px`;
    dataTableContainer.style.height = `${window.innerHeight - filtersHeight}px`;
    verticalScrollBarContainer.style.width = `${window.innerHeight - filtersHeight}px`;


    verticalScrollBarContainer.style.left = `${window.innerWidth - 
      (window.innerHeight - filtersHeight +
        verticalScrollBarContainer.offsetHeight) / 2}px`;

    const availibleVerticalSpace = window.innerHeight - filtersHeight - // dataTableContainer.offsetHeight -
      (headerHeight + footerHeight);
    console.log('App.js --> initializeDataTable: availibleVerticalSpace = ' + availibleVerticalSpace);
    const batchSize = Math.floor(availibleVerticalSpace / (footerHeight + 2));
